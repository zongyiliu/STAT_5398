...
lora_module_dict = {
    ...
    'llama31': [  # Llama-3.1-8B support
        'q_proj', 'k_proj', 'v_proj',
        'o_proj', 'gate_proj', 'up_proj', 'down_proj',
    ],
    'deepseek': [  # DeepSeek-R1-Distill-Llama-8B support
        'q_proj', 'k_proj', 'v_proj',
        'o_proj', 'gate_proj', 'up_proj', 'down_proj',
    ],
}
...
def parse_model_name(name, from_remote=False):
    ...
    if name == 'chatglm2':
        ...
        return 'meta-llama/Llama-3.1-8B'  # Llama-3.1-8B support
    elif name == 'deepseek':
        return 'deepseek-ai/DeepSeek-R1-Distill-Llama-8B'  # DeepSeek-R1 support
    else:
...
def load_dataset(names, from_remote=False):
    """
    简化版本：直接使用传入的路径，不做任何修改
    对于绝对路径，添加 file:// 协议避免 fsspec 误判
    """
    ...
    for name in dataset_names:
        ...
        if from_remote:
            ...
            # 如果是绝对路径（以 / 开头），使用 file:// 协议
            if name.startswith('/'):
                # 对于绝对路径，fsspec 需要 file:// 协议
                tmp_dataset = datasets.load_from_disk(f"file://{name}")
            else:
                # 相对路径直接使用
