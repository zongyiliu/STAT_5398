...
# Add LoRA module mapping for new models
lora_module_dict = {
    "chatglm2": ["query_key_value"],
    "llama2": ["q_proj", "k_proj", "v_proj", "o_proj", "up_proj", "down_proj", "gate_proj"],
    "llama3.1": ["q_proj", "k_proj", "v_proj", "o_proj", "up_proj", "down_proj", "gate_proj"],
    "deepseek": ["q_proj", "k_proj", "v_proj", "o_proj", "up_proj", "down_proj", "gate_proj"]
}
...
class GenerationEvalCallback(TrainerCallback):
    def __init__(self, eval_dataset, tokenizer=None, model=None, ignore_until_epoch=0):
    ...
    def on_evaluate(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if state.is_local_process_zero:
            ...
            tokenizer = self.tokenizer or kwargs.get("tokenizer")

            if model is None or tokenizer is None:
                print("⚠️ Skip GenerationEvalCallback: tokenizer/model not found.")
                return
...
def main(args):
    ...
    print('⚠️ Skipping exceed_max_length filtering (using truncation instead)')
    ...
    peft_config = LoraConfig(
        ...
        bias='none',
        # ✅ 禁用 bitsandbytes / 4bit 依赖
        use_dora=False,      # 只用标准LoRA
        use_rslora=False,    # 不用 rank-stabilized LoRA
        init_lora_weights="gaussian",  # 正常初始化     
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_model", type=str, choices=[
        'chatglm2', 'llama2', 'llama3.1', 'deepseek'
    ])
    ...
    if not getattr(args, "base_model", None):
        args.base_model = "llama3.1"
    if not getattr(args, "dataset", None):
        args.dataset = "fingpt-forecaster-dow30-202305-202405"
