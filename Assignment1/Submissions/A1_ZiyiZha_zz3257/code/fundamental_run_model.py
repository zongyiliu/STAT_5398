...
if __name__ == '__main__':
    ...
    parser.add_argument('--tic_column', type=str, default='tic',
                    help='column name used as the security identifier (e.g., tic or gvkey)')
   ...
   # fundamental_total=fundamental_total[fundamental_total['tradedate'] < 20170901]
    ...
    # first_trade_date_index = args.first_trade_index
    # only use data from 2018 and later
    unique_datetime = sorted(sector_data.date.unique())
    first_trade_date_index = next(i for i, d in enumerate(unique_datetime) if str(d) >= "2018-01-01")
