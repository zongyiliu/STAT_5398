...
daily_filename = "./sp500_tickers_daily_price_20250712.csv" ### Replace with your own daily stock information file name
selected_stock_filename = "./stock_selected.csv" ### Replace with your own selected stocks file name
weights_filename = "./portfolio_submission.csv" ### Replace with your own stock weights file name
enddate = "2025-10-02"
...
if __name__ == "__main__":
    ...
    usefulcolumns = ["datadate", "prcod", "prccd", "ajexdi", "gvkey"]
    ...
    last_weights = None  # 保存上一期权重
    
    for date in tqdm(dates, desc="Backtesting"):
        # 当前日期的数据
        cur_info = dailyinfo[dailyinfo["datadate"] == date].set_index("gvkey")
    
        # 如果是调仓日
        if date in tradedates:
            cur_weights = weights[weights["trade_date"] == date][["gvkey", "weights"]].set_index("gvkey")
            last_weights = cur_weights.copy()  # 记录最新权重
            port.daily_update(date, cur_weights, cur_info, rebalancing=True)
    
        # 如果不是调仓日，但已有上一期权重
        elif last_weights is not None:
            port.daily_update(date, last_weights, cur_info, rebalancing=False)
    ...
    min_len = min(len(dates), len(port_mvs_quart), len(SP500), len(QQQ))
    dates = dates[:min_len]
    port_mvs_quart = port_mvs_quart[:min_len]
    SP500 = SP500[:min_len]
    QQQ = QQQ[:min_len]
    
    ax.plot(dates, [data / port_mvs_quart[0] for data in port_mvs_quart],
            label='Portfolio Value with Quarterly Adjustment', color='blue', linewidth=2)
    ax.plot(dates, [data / SP500[0] for data in SP500],
            label='SP500', color='green', linewidth=2)
    ax.plot(dates, [data / QQQ[0] for data in QQQ],
            label='QQQ', color='red', linewidth=2)
