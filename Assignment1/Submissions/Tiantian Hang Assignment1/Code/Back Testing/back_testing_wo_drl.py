...
# -*- coding: utf-8 -*-
"""
back_testing_wo_drl.py
Single-file CLI tool to backtest and compare SPX, QQQ, Mean-Variance, Minimum-Variance,
and Equally Weighted portfolios with unified KPI computation (without DRL).
...
- NEW: Rolling comparison charts for all KPIs across SPX, QQQ, MeanVar, MinVar, Equal
"""
...
def load_equity_prices(path: str, gvkey_col="gvkey", date_col="datadate",
    ...
    if "date" in df.columns and date_col == "datadate":
        date_col = "date"  # Use 'date' if available and expected 'datadate'
    
    # Check if adj_close_q already exists (for final_ratios.csv)
    if "adj_close_q" in df.columns:
        # Use pre-calculated adjusted close
        for c in [gvkey_col, date_col]:
        ...
        # Calculate adjusted close from prccd and ajexdi
...
def align_calendar_forward_fill(objs: Union[List[pd.DataFrame], List[pd.Series]]) -> Union[List[pd.DataFrame], List[pd.Series]]:
...
def generate_comparison_charts(daily_returns_map: Dict[str, pd.Series], out_dir: str,
    ...
    """
    Build rolling comparison charts for all KPIs across SPX, QQQ, MeanVar, MinVar, Equal.
    Produces charts for frequencies M/Q/Y using default rolling windows.
    """
    labels = ["SPX","QQQ","MeanVar","MinVar","Equal"]
    for freq, win in _ROLL_WINDOWS.items():
        ...
        for metric_name, df in [
            ...
            df = df[ [c for c in ["SPX","QQQ","MeanVar","MinVar","Equal"] if c in df.columns] ]
...
# ------------------------------- Main --------------------------------
def main():
    parser = argparse.ArgumentParser(description="Backtest & KPI tool for classic portfolios (without DRL).")
    ...
    parser.add_argument("--equity-file", type=str, default="./outputs/final_ratios.csv")
    ...
    parser.add_argument("--anchor", type=str, default="MeanVar", choices=["MeanVar","MinVar","Equal"],
    ...
    weights_map = {"MeanVar": w_mean, "MinVar": w_min, "Equal": w_equal}
    ...
    summary_rows = []
    metrics_map = {}
    for label in ["MeanVar","MinVar","Equal","SPX","QQQ"]:
