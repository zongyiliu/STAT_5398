...
def run_stock_selection(data_path, output_path):
    ...
    # Create tic-to-gvkey mapping from fundamental data
    print("Creating tic-to-gvkey mapping from fundamental data...")
    try:
        fund_data = pd.read_csv(FUNDAMENTAL_FILE, usecols=['tic', 'gvkey'])
        tic_to_gvkey = dict(zip(fund_data['tic'], fund_data['gvkey']))
        print(f"✓ Created mapping for {len(tic_to_gvkey)} unique tic-gvkey pairs")
    except Exception as e:
        print(f"Error creating tic-to-gvkey mapping: {str(e)}")
        sys.exit(1)
    
    # tic is unique identifier, also include gvkey for portfolio construction
    df_dict = {'tic': [], 'gvkey': [], 'predicted_return': [], 'trade_date': []}
    ...
    for sector in sectors:
        ...
        cmd = f'"/Users/tiantian/Downloads/CU Material/S2/AI4Finance/.venv/bin/python" fundamental_run_model.py -sector_name sector{sector} -tic_column tic -fundamental {FUNDAMENTAL_FILE} -sector {sector_file}'
        ...
        try:
            ...
            for idx in df.index:
                ...
                for tic in predicted_return.index:
                    # Get corresponding gvkey for this tic
                    gvkey = tic_to_gvkey.get(tic, None)
                    if gvkey is not None:
                        ...
                        print(f"    Warning: No gvkey found for tic {tic}, skipping")
    ...
    if len(df_result) > 0:
        ...
        print(f"  Unique stocks (tic): {df_result['tic'].nunique()}")
        print(f"  Unique stocks (gvkey): {df_result['gvkey'].nunique()}")
